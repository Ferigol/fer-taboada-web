...
export type Section = {
  ...
  renderOverlay?: (active: boolean, previous: boolean) => ReactNode;
};
  ...
  (
    {
      ...
      className,
      style,
      fontFamily = 'var(--font-gilroy), "Oswald", "Arial Black", sans-serif',
      header,
      footer,
      gap = 1,
      gridPaddingX = 2,
    ...
    const changeSection = (to: number) => {
      ...
      leftItemRefs.current.forEach((el, i) => {
        const hasLabel = !!sections[i]?.leftLabel;
        el.classList.toggle("active", i === to && hasLabel);
        gsap.to(el, {
          opacity: i === to && hasLabel ? 1 : 0.3,
          x: i === to && hasLabel ? 10 : 0,
          duration: D * 0.6,
          ease: "power3.out",
        });
      });
      rightItemRefs.current.forEach((el, i) => {
        const hasLabel = !!sections[i]?.rightLabel;
        el.classList.toggle("active", i === to && hasLabel);
        gsap.to(el, {
          opacity: i === to && hasLabel ? 1 : 0.3,
          x: i === to && hasLabel ? -10 : 0,
    ...
    const handleLoadedStagger = () => {
      leftItemRefs.current.forEach((el, i) => {
        gsap.fromTo(
          el,
          { opacity: 0, y: 20 },
          { opacity: i === index ? 1 : 0.3, y: 0, duration: 0.5, delay: i * 0.06, ease: "power3.out" }
        );
      });
      rightItemRefs.current.forEach((el, i) => {
        gsap.fromTo(
          el,
          { opacity: 0, y: 20 },
          { opacity: i === index ? 1 : 0.3, y: 0, duration: 0.5, delay: 0.2 + i * 0.06, ease: "power3.out" }
        );
      });
    };
    ...
    const cssVars: CSSProperties = {
      ...
      ["--fx-page-bg" as any]: colors.pageBg ?? "#0d0d0d",
      ["--fx-stage-bg" as any]: colors.stageBg ?? "#0d0d0d",
      ["--fx-accent" as any]: "#ff5a00",
      ...
      >
        ...
        <div className="fx-scroll">
          <div className="fx-fixed-section" ref={fixedSectionRef}>
            <div className="fx-fixed" ref={fixedRef}>
              ...
              {sections.map((s, i) =>
                s.renderOverlay ? (
                  <div key={`OV-${s.id ?? i}`} className="fx-overlay-slot">
                    {s.renderOverlay(index === i, lastIndexRef.current === i)}
                  </div>
                ) : null
              )}
                  ...
                  <div className="fx-left" role="list">
                    <div className="fx-track" ref={leftTrackRef}>
                      {sections.map((s, i) => (
                        <div
                          key={`L-${s.id ?? i}`}
                          className={`fx-item fx-left-item ${i === index && s.leftLabel ? "active" : ""}`}
                  ...
                  <div className="fx-right" role="list">
                    <div className="fx-track" ref={rightTrackRef}>
                      {sections.map((s, i) => (
                        <div
                          key={`R-${s.id ?? i}`}
                          className={`fx-item fx-right-item ${i === index && s.rightLabel ? "active" : ""}`}
              ...
              {showProgress && (
                <div className="fx-progress-outer">
        ...
        <style jsx>{`
          ...
          .fx-bg-overlay {
            ...
            background: linear-gradient(to bottom, rgba(13,13,13,0.5) 0%, rgba(13,13,13,0.2) 50%, rgba(13,13,13,0.75) 100%);
          }
          ...
          .fx-progress {
            width: 200px; height: 1px; margin: 1.5rem auto 0;
